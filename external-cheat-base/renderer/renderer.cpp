...
void renderer::fpss(clock_t fps) {
	LPD3DXFONT mFont = NULL; // Define a global or class member variable
	// During initialization
	D3DXCreateFont(renderer::pDevice, 22, 0, FW_NORMAL, 1, FALSE, DEFAULT_CHARSET,
		OUT_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE,
		TEXT("Arial"), &mFont);
	// Within your rendering loop
	RECT rc;
	SetRect(&rc, 10, 10, 0, 0); // Position for the text
	mFont->DrawText(NULL, std::to_wstring(fps).c_str(), -1, &rc, DT_NOCLIP, D3DCOLOR_ARGB(255, 255, 255, 255));
	// During cleanup
	if (mFont) {
		mFont->Release();
		mFont = NULL;
